    ...
    <div class="wrap_cont">
    	<c:choose>
    		<c:when test="${optionResult eq 1 }">
	            ...
	            <h2>转换成功!</h2>
            	<div class="state_btn"><a href="<c:url value = '/assetes/toAssetes?flag=2' />">返回货币转换首页</a></div>
        		</div>
    		</c:when>
    		<c:when test="${optionResult eq 2 }">
    			<div class="state_box fail">
		            ...
		            <h2>${faildMessage }</h2>
		            <div class="state_btn"><a href="<c:url value = '/assetes/gotoTransform' />">返回转换页面</a></div>
	        	</div>
    		</c:when>
    		<c:otherwise>
    			<div class="state_box succ">
